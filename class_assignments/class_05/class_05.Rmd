...
#There are more records in buyer_annual_by_year than in buyer_totals because buyer_annual includes one observation for each year (where applicable) between 2006 and 2012 for each pharmacy, whereas Bbuyer_totals includes just one record per pharmacy

#There are more records in buyer_addresses than in buyer_totals because some pharmacies have more than one location, therefore they have different records in buyer_addresses.
...
```{r}
totals_and_addresses <- buyer_totals %>%
  inner_join(buyer_addresses, by="buyer_dea_no") %>%
  arrange(desc(total_pills))

#VA CONSOLIDATED MAIL OUTPATIENT PHARMACY in North Charleston had the most pills sent to it in total, likely because it is one of only a handful of mail order pharmacies for the U.S. Dept of Veterans Affairs.
```
...
annuals_and_addresses <- buyer_annual_by_year %>%
  inner_join(buyer_addresses, by="buyer_dea_no") %>%
...
#Dr.Rakesh Mathur received the most pills in Maryland in 2010. He received 337,450, more than 100,000 more than the next closest doctor. He was put on probation and fined several thousand dollars. I would try and contact the DEA to verify he is the same person in ARCOS.
...
```{r}
options(scipen= 999)
pills_per_state <- buyer_totals %>%
  group_by(buyer_state) %>%
summarise(pills_per_state =sum(total_pills))

pills_per_person <- pills_per_state %>%
 full_join(state_population_per_year, by="buyer_state")%>%
mutate(pills_per_person = pills_per_state/population_average)%>%
...
```{r}
buyer_inner<- buyer_totals %>%
  inner_join(buyer_addresses, by="buyer_dea_no")

buyer_left <- buyer_totals %>%
  left_join(buyer_addresses, by="buyer_dea_no")
#same number of observations, which must mean it's the buyer_addresses table that has extra values that aren't getting matched up.

buyer_left%>%
  filter(is.na(buyer_address1))
#no rows returned because all buyers have an address listed in this columnn
...
anti_join(buyer_addresses, buyer_totals, by = "buyer_dea_no")
#flipped the order. that showed me null values in columns like buyer_address2 and buyer_addl_co_info, which I wasn't necessarily looking for.

buyer_left <- buyer_addresses %>%
  left_join(buyer_totals, by="buyer_dea_no")
#so i flipped the order on the left join from above, then i could filter it out by only "buyer_address1"
