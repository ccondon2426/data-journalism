...
```{r}
# The records that appear in the not_in_population are from counties Puerto Rico or the U.S. Virgin Islands, meaning these regions can't be included in our map.
```
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="West Virginia received most pills per person in 2009", subtitle = "2009 pain pills per person", caption = "Source: Washington Post ARCOS") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person increased over time in many states", subtitle = "Pills received by each state, divided by population, between 2006 and 2012", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
```{r}
pills_population_2009 <- pills_population %>%
  filter(year == "2009")

pills_per_person_pop_2009<- pills_population_2009 %>%
...
#county_geodata_modified <- county_geodata_shifted %>%
#mutate(statefips = str_sub(GEOID, 1,2)) %>%
#select(statefips, GEOID, NAME, variable, estimate, moe, geometry)

geo_pills_population_county_2009 <- county_geodata_shifted %>%
inner_join(pills_per_person_pop_2009, by=c("GEOID"="countyfips"))

mapview(geo_pills_population_county_2009, zcol = "pills_per_person", legend = TRUE)

#In addition to the higher number of pills per person in Appalachia, there were also some high numbers in Nevada and Northern California. There's also one county with a huge number of pills per person, Charleston County, South Carolina.
```
...
```{r}
pills_per_person_md<- arcos_county_pills_per_year %>%
  filter(buyer_state == "MD")

md_counties <- pills_per_person_md %>%
inner_join(arcos_county_population_per_year, by = c("countyfips","buyer_county", "buyer_state","year")) 

md_counties_pills_per_person <- md_counties%>%
  mutate(pills_per_person=dosage_unit/population)

maryland_counties <- county_geodata_shifted %>%
  inner_join(md_counties_pills_per_person, by = c("GEOID" = "countyfips"))
...
maryland_counties %>%
 ...
 geom_sf(lwd = 0) +
 theme_map() +
 labs(fill='Pills per Person',title="Opioids were at their worst in Alleghany, Cecil, and Kent Counties", subtitle = "Opioids per person by county, 2006-2012", caption = "Source: ARCOS database") +
 theme(legend.position="bottom") +
 scale_fill_viridis_c(option = "plasma",labels = comma)

#It seems the most opioids per person were shipped into communities on the Eastern and Western edges, in addition to the Baltimore area, but much of the areas in the middle of the state fared better.
```
