...
```{r}
bco_raw <- summarized_county_monthly(county="Baltimore", state = "MD", key = key)
```

```{r}
bco_distributors <- total_distributors_county(county="Baltimore", state = "MD", key = key)
```

```{r}
timonium_buyers <- buyer_addresses(county="Baltimore", state= "MD", key = key) %>%
  filter(BUYER_CITY == "TIMONIUM")
```
## Analysis
...
```{r}
#In what month during this period did Baltimore County receive the most pills? 
bco_raw %>%
...
```{r}
#What pharmacy in Timonium received the most pills during this period?
timonium_buyers <- clean_names(timonium_buyers)
timonium_buyers %>%
inner_join(buyer_totals, by = "buyer_dea_no") %>%
arrange(desc(total_pills))
#Lykos Pharmacy off of York Road
```

```{r}
#What city in Baltimore County received the most pills?


bco_totals <- buyer_addresses(county="Baltimore", state = "MD", key = key)
bco_totals <- clean_names(bco_totals)
bco_totals %>%
inner_join(buyer_totals, by = "buyer_dea_no") %>%
select("buyer_city", "total_pills") %>%
group_by(buyer_city) %>%
  summarise(dosage_unit = sum(total_pills)) %>%
arrange(desc(dosage_unit))

#Baltimore followed by towson
```

```{r}
#I noticed that Lutherville Timonium appears twice. Once with a dash, and once without. How does the city rank if it is named properly?
...
group_by(buyer_city) %>%
  summarise(dosage_unit = sum(total_pills)) %>%
arrange(desc(dosage_unit)) %>%
   mutate(buyer_city = case_when(buyer_city == "LUTHERVILLE TIMONIUM" ~ "Lutherville-Timonium",
        buyer_city == "LUTHERVILLE" ~ "Lutherville-Timonium",
        buyer_city == "LUTHERVILLE-TIMONIUM" ~ "Lutherville-Timonium",
        TRUE ~ buyer_city)) %>%
group_by(buyer_city) %>%
...
```{r}
#What pharmacy in Baltimore County received the most pills?
bco_totals %>%
inner_join(buyer_totals, by = "buyer_dea_no") %>%
select("buyer_name", "buyer_address1", "total_pills") %>%
arrange(desc(total_pills))
#Drug City Pharmacy near Dundalk
```
## Output
