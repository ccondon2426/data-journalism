...
Now, we want to get an average number of pills per CoC between 2006 and 2012, and we want to divide that by the 2010 population.
...
homelessness_rate_pills_per_person <- coc_pills_per_person_avg %>%
  ...
  select(coc_code, coc_name, avg_pills, total_2010_population, pills_per_capita, overall_homeless_2014) %>%
...
ggplot(homelessness_rate_pills_per_person) +
 ...
 geom_smooth(aes(homelessness_rate, pills_per_capita), method = "lm", se = FALSE) +
   geom_text_repel(aes(homelessness_rate, pills_per_capita, label=coc_name),
            subset(homelessness_rate_pills_per_person, pills_per_capita > 10))

```

In this graph, you can see mendocino county is a pretty big outlier, with both a high rate of pills per person and a high homelessness rate. So this will be an area of interest going forward. (See below)
