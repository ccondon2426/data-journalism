...
#Biggest combo = Actavis Pharma, Inc. -> Miami-Luken -> Tug Valley Pharmacy, LLC
```
...
```{r}
mingo %>%
  group_by(combined_labeler_name, reporter_name, reporter_city, reporter_address1, buyer_name) %>%
  summarise(pills_per_grouping = sum(dosage_unit)) %>%
...
#Essentially, I want to utilize the unite function but for values in a column rather than columns themselves. I want to unite the values "CARDINAL HEALTH", "CARDINAL HEALTH, INC.", and "CARDINAL HEALTH 110, LLC". But since I'm not sure how to do that, I could try using the case_when function, and setting it so that I create a new column, in which any value that isn't one of those three receives a "not_cardinal_health" output, and any of those three received a "cardinal_health" output. That way I have a new row where these three companies are united into one.
